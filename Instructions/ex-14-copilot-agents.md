...
You must have completed **Exercises 01 and 14**. Copilot data movement must be enabled, and at least one published knowledge article must exist (the LCD Screen Troubleshooting article from Exercise 14).
...
1. Confirm that the **Copilot for Service** toggle is set to **On**. If it is not, enable it and select **Save**.

1. Confirm that **Cross-region data movement** is enabled. If not, return to **Power Platform admin center** → **Contact Center Trial** environment → **Settings** → **Features** and enable it.
...
1. In the **Instructions** field, enter the following custom prompt instructions:
   ...
   ```
    ...
    > **Note**: Custom instructions tell Copilot how to behave when responding to users. By specifying the tone, format, and escalation guidance here, you ensure that every Copilot response is consistent with Contoso Coffee's support standards — without agents needing to craft detailed prompts themselves. Instructions are applied to all **Ask a question** responses generated from the Dynamics 365 knowledge base.

1. In the **Knowledge sources** section, select the **Include a knowledge base or another type of knowledge source** checkbox.

1. Select **Use your organization's knowledge base as knowledge source**.

1. Select **Save**.

    > **Note**: This enables Copilot to use your published Dynamics 365 knowledge articles to answer agent questions and draft responses. The number of articles currently indexed is displayed next to the option.
...
1. Select **Save**.
    ...
    > **Note**: Features not enabled here will not appear in the agent's workspace, even if they are configured globally. Enabling only the features your agents need keeps the workspace focused and reduces distraction.
...
1. Open a new browser tab and go to [make.powerapps.com](https://make.powerapps.com).

1. Confirm the **Contact Center Trial** environment is selected in the top-right corner.

1. In the left navigation, select **AI hub**, then select **AI prompts**.

    > **Note**: If you don't see **AI hub** in the left navigation, select **More** at the bottom of the nav to find it.

1. Select **Prompts** and then select **Build your own prompt.**
...
1. On the **Plugin data** screen, select **Store this plugin's data**, then select **Turn on plugin**.

1. When prompted, select **Publish changes**, then select **Confirm**.

    > **Note**: It can take up to 15 minutes for the plugin to become available to agents. Agents access it from the Copilot pane by typing `/` to browse available plugins.

## Bonus tasks (complete if available in your trial)

### Bonus Task A - Configure the Quality Assurance Agent

> **Note**: The QA Agent evaluates conversations for quality scoring. Verify it is available in your trial before proceeding.

1. In **Copilot Service admin center**, select **Quality Assurance Agent**.

1. Enable the agent and configure connection references if prompted.

1. Set a **scoring threshold** of `70` for chat conversations.

1. Select **Save**.

### Bonus Task B - Configure the Service Operations Agent

> **Note**: The Service Operations Agent provides conversational configuration assistance and orchestrates playbooks.
